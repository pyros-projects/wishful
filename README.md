...
**Be honest about what this is.** Generated code executes **in your process**. A static AST scan stops a careless generation, not a determined one — *reflective* access through library indirection (`operator.attrgetter(...)`, an arbitrary mapping's `.get("__builtins__")`) can still slip through, because AST scanning cannot model runtime values. Only an out-of-process sandbox would make running model-written code truly safe. Treat the validator as a seatbelt, not a vault. For untrusted inputs, review the cached code (it's plain Python) or run wishful where arbitrary code execution is acceptable.
