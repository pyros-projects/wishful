...
- Hardened safety validator, framed honestly as **"defense in depth, not a sandbox"**: AST scan blocks forbidden imports/calls, `__builtins__`/`globals()`/`vars()` gadget access, write-mode `open()`, and system calls through aliases. The same scan re-runs on cache load, so tampered `.wishful/` files are re-checked.

#### 🗄️ Cache
- **Cache integrity + namespacing**: `static` and `dynamic` prefixes map to the same cache file; cached sources are re-validated on load.

#### 🔍 Review gate
- **Review-gate ordering + headless guard**: review happens before execution, with a guard for non-interactive environments.

#### 🎨 Type Registry
- Type-registry fixes for serialization and output-type binding.

#### 📦 Versioning
- **`__version__` now derives from installed package metadata** (`importlib.metadata.version`) instead of being hardcoded.

### Dependencies
- Refreshed: `litellm`, `rich`, `python-dotenv`, `pydantic`.
