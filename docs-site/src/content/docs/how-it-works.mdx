...
#### The dynamic attribute contract

Attribute access on a dynamic module is **lazy and free**: it returns a callable
wrapper without spending an LLM call. Generation happens only when you *invoke*
the wrapper. Three consequences are intentional:

- `hasattr(mod, name)` is `True` for every public name — the module can't know
  what the model would generate without paying for a generation, so it doesn't try.
- `dir(mod)` only lists what previous generations produced, not what's possible.
- Dynamic modules expose **functions only**. If a call's regeneration produces the
  name as a non-callable (a constant), the call raises `AttributeError`; if the
  model fails to produce the requested symbol at all, the existing module and
  snapshot are left untouched. Use `wishful.static.*` for modules with data
  attributes.
...
- `wishful.dynamic.foo` → `.wishful/_dynamic/foo.py` (a disjoint namespace — dynamic snapshots never collide with static cache files)
